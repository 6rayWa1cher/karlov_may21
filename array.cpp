...
Array::Array(const Array &x) : size(x.size), capacity(x.capacity),
                               arr(clone(x.arr, x.size)){
    
}

Array &Array::operator=(const Array &rhs) {
    if (&rhs != this) {
        auto* newArray = clone(rhs.arr, rhs.capacity, rhs.size);
...
uint8_t Array::add(uint8_t v) {
    if (size == capacity) {
        ...
        auto* newArr = clone(arr, extendedCapacity, size);       
...
void Array::removeByIndex(uint32_t i) {
    ...
    if (reducedCapacity >= size && reducedCapacity >= 10) {
        auto* newArray = clone(arr, reducedCapacity, size);
